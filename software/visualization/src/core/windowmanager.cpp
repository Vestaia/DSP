...
int WindowManager::initializeWindow(const char* windowName, int windowWidth, int windowHeight) {
    // Setup GLFW window
    if (!glfwInit())
        return 1;
...
void WindowManager::setupVulkan(const char** extensions, uint32_t extensions_count) {
...
void WindowManager::setupVulkanWindow(VkSurfaceKHR surface, int width, int height) {
    wd->Surface = surface;
...
void WindowManager::glfw_framebuffer_size_callback(GLFWwindow* window, int width, int height) {
    ...
    obj->glfw_framebuffer_size_callback(width, height);
}
...
void WindowManager::checkSwapChainRebuild() {
    // Resize swap chain?
    if (g_SwapChainRebuild)
    {
        int width, height;
